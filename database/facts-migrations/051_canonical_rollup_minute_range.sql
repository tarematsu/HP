-- Daily rollups are keyed by the fact's canonical minute. observed_at records
-- when a fact was received and can put historical/backfill facts into the current
-- ingestion day, turning a one-day rollup into a multi-million-row scan.
--
...
SELECT f.id,f.minute_at AS observed_at,f.channel_id,NULL AS channel_alias,NULL AS channel_name,
