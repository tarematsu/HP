...
void StationheadPlayer::EvaluateAudioLossRecovery(int64_t nowMs) {
  ...
  if (audioPlaying) {
    ...
    // Authentication state owns the interaction surface. Audible playback can
    // continue while Stationhead is asking the user to log in, so audio alone
    // must never clear a confirmed loginRequired_ latch. Once authentication
    // has actually cleared that latch, ordinary playback can return behind the
    // dashboard as before.
