...
<div class="row" style="margin-top:12px"><button id="load" class="primary">クラウド設定を読み込む</button><button id="save" class="primary">保存</button></div><div id="status" class="status"></div><div class="muted">Spotifyは spotify.rotation の fixed / shuffle / random ブロックを上から順に再生します。tracks配列は任意件数、randomのみcountで1サイクルに採用する曲数を指定できます。TALKABOUTも spotify.talkAbout で管理します。端末への反映は次回クラウド同期時です。再生中の曲は継続し、ローテーション変更は次の曲から反映します。</div></section>
