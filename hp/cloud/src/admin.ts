...
<div class="row" style="margin-top:12px"><button id="load" class="primary">クラウド設定を読み込む</button><button id="save" class="primary">保存</button></div><div id="status" class="status"></div><div class="muted">Spotifyは spotify.rotation の fixed / shuffle / random ブロックを上から順に再生します。tracks配列は任意件数、randomのみcountで1サイクルに採用する件数を指定できます。同一サイクルでは同じSpotify pathを重複採用しません。端末への反映は次回クラウド同期時です。</div></section>
...
const migrate=config=>{const output=structuredClone(config||{}),station=output.stationhead;if(station&&typeof station==="object"&&!Array.isArray(station)){if(!Object.hasOwn(station,"blockImages")&&Object.hasOwn(station,"blockImagesAfterPlayback"))station.blockImages=station.blockImagesAfterPlayback;if(!Object.hasOwn(station,"blockFonts")&&Object.hasOwn(station,"blockFontsAfterPlayback"))station.blockFonts=station.blockFontsAfterPlayback;delete station.blockImagesAfterPlayback;delete station.blockFontsAfterPlayback;delete station.hideChatAfterPlayback;delete station.lowMemoryMode;delete station.memoryLimitMb}const spotify=output.spotify;if(spotify&&typeof spotify==="object"&&!Array.isArray(spotify)){for(const key of Object.keys(spotify)){if(key!=="managedRotation"&&key!=="rotation")delete spotify[key]}}const rotation=output.spotify?.rotation;if(Array.isArray(rotation)&&rotation.length===3){const first=rotation[0],middle=rotation[1],random=rotation[2],firstIds=trackIds(first),middleIds=trackIds(middle),randomIds=trackIds(random);if(first?.mode==="fixed"&&middle?.mode==="shuffle"&&random?.mode==="random"&&Number(random?.count??1)===1&&firstIds.length===1&&firstIds[0]==="6Vy6hCA2CZwZalGqaX6Sew"&&sameTrackSet(middleIds,legacySpotifyMiddleTrackIds)&&managedRandomPool(randomIds))output.spotify.rotation=structuredClone(managedSpotifyRotation)}return output};
