...
const migrate=config=>{const output=structuredClone(config||{}),station=output.stationhead;if(station&&typeof station==="object"&&!Array.isArray(station)){if(!Object.hasOwn(station,"blockImages")&&Object.hasOwn(station,"blockImagesAfterPlayback"))station.blockImages=station.blockImagesAfterPlayback;if(!Object.hasOwn(station,"blockFonts")&&Object.hasOwn(station,"blockFontsAfterPlayback"))station.blockFonts=station.blockFontsAfterPlayback;delete station.blockImagesAfterPlayback;delete station.blockFontsAfterPlayback;delete station.hideChatAfterPlayback}const talkAbout=output.spotify?.talkAbout;if(talkAbout&&typeof talkAbout==="object"&&!Array.isArray(talkAbout))delete talkAbout.intervalMinutes;const rotation=output.spotify?.rotation;if(Array.isArray(rotation)&&rotation.length===3){const first=rotation[0],middle=rotation[1],random=rotation[2],firstIds=trackIds(first),middleIds=trackIds(middle),randomIds=trackIds(random);if(first?.mode==="fixed"&&middle?.mode==="shuffle"&&random?.mode==="random"&&Number(random?.count??1)===1&&firstIds.length===1&&firstIds[0]==="6Vy6hCA2CZwZalGqaX6Sew"&&sameTrackSet(middleIds,legacySpotifyMiddleTrackIds)&&managedRandomPool(randomIds))output.spotify.rotation=structuredClone(managedSpotifyRotation)}return output};
