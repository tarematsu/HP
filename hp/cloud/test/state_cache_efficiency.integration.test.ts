...
describe("state cache and D1 efficiency", () => {
  ...
  it("suppresses unchanged current_state heartbeat writes for twenty-four hours", async () => {
    const startedAt = Date.now();
    vi.useFakeTimers();
    vi.setSystemTime(startedAt);
    ...
    await updateState(env, { source: "weather", observedAt: startedAt + 23 * 60 * 60_000, payload });
    const suppressed = await readState(env, "weather");
    expect(suppressed?.fetched_at).toBe(initial?.fetched_at);

    vi.setSystemTime(startedAt + 24 * 60 * 60_000 + 60_000);
    await updateState(env, { source: "weather", observedAt: startedAt + 24 * 60 * 60_000 + 60_000, payload });
    const written = await readState(env, "weather");
    expect(written?.fetched_at).toBe(startedAt + 24 * 60 * 60_000 + 60_000);
  });
});
