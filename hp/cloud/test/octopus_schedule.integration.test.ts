...
describe("Octopus schedule", () => {
  it("normalizes the Octopus job to a 12-hour interval", async () => {
    vi.useFakeTimers();
    ...
    expect(job?.interval_seconds).toBe(43_200);
  ...
  it("preserves an already queued immediate refresh", async () => {
    ...
    expect(job).toEqual({ interval_seconds: 43_200, next_run_at: 0 });
  });
});
