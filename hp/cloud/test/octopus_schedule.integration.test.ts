...
describe("Octopus schedule", () => {
  it("normalizes the Octopus job to a 3-hour interval", async () => {
    vi.useFakeTimers();
    ...
    expect(job?.interval_seconds).toBe(10_800);
  ...
  it("preserves an already queued immediate refresh", async () => {
    ...
    expect(job).toEqual({ interval_seconds: 10_800, next_run_at: 0 });
  });
});
