...
test('Stationhead can bring an offscreen Start Listening control into the compact viewport before CDP click', () => {
  ...
  const pointStart = locator.indexOf('const pointOf = (target, label = target) =>', renderedStart);
  ...
  assert.match(pointOf, /label\.scrollIntoView\(/);
  assert.match(pointOf, /rect = label\.getBoundingClientRect\(\)/);
