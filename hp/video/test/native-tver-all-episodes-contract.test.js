...
test('TVer natural completion is captured from the active program media only', () => {
  assert.match(runtime, /state\.programKey/);
  ...
  assert.doesNotMatch(
    runtime,
    /episodeMaxPlaybackMs|episodeStartedAt|episodeLimitTimer|postrollGraceMs/,
  );
});
...
test('TVer observation stays player-scoped and event-driven', () => {
  assert.match(runtime, /state\.playerObserver = new MutationObserver/);
