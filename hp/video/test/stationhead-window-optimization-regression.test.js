...
test('normal background state keeps a full host with a fixed 960x360 playback viewport', () => {
  ...
  assert.match(layoutSource, /kStationheadPlaybackViewportWidth = 960/);
  assert.match(layoutSource, /kStationheadPlaybackViewportHeight = 360/);
  assert.match(layoutSource, /kStationheadWindowNameHeight = 24/);
