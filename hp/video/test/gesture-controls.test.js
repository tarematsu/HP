...
test('screen orientation overrides stale WebView viewport dimensions', () => {
  assert.equal(isLandscapeLayout(390, 844, 'landscape-primary'), true);
  assert.equal(isLandscapeLayout(844, 390, 'portrait-primary'), false);
});

test('viewport dimensions remain the fallback when orientation metadata is unavailable', () => {
  assert.equal(isLandscapeLayout(844, 390, ''), true);
  assert.equal(isLandscapeLayout(390, 844, ''), false);
});

test('landscape controls use horizontal navigation and vertical seeking', () => {
  assert.deepEqual(gestureAxes(isLandscapeLayout(390, 844, 'landscape-primary')), {
    nextAxis: 'x',
    seekAxis: 'y'
  });
});
