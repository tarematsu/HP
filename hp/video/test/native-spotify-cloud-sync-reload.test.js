...
test('natural scheduler wakes throttle synchronized playlist cache checks', () => {
  ...
  assert.match(header, /kSpotifyCloudReloadMinIntervalMs = 60ULL \* 1000ULL/);
  ...
  assert.match(
    run,
    /if \(!cloudPlaylistWasReady\) \{[\s\S]*nextCloudPlaylistCheckTick_ = now \+ kSpotifyCloudReloadMinIntervalMs/,
  );
  assert.match(
    run,
    /else if \(nextCloudPlaylistCheckTick_ == 0 \|\|[\s\S]*now >= nextCloudPlaylistCheckTick_\) \{[\s\S]*EnsureCloudPlaylistLoaded\(\);[\s\S]*nextCloudPlaylistCheckTick_ = now \+ kSpotifyCloudReloadMinIntervalMs/,
  );
