...
<div class="row" style="margin-top:12px"><button id="load" class="primary">クラウド設定を読み込む</button><button id="save" class="primary">保存して端末を再起動</button></div><div id="status" class="status"></div><div class="muted">Cloudflare同期は60秒、CO2・温湿度アップロードは30分で固定されます。Spotifyは端末から直接取得します。</div></section>
...
const defaults={cloudPollSeconds:60,telemetryMinutes:30,screen:{width:1280,height:800},co2:{serialPort:"",temperatureOffset:-4.5},stationhead:{url:"https://www.stationhead.com/c/buddies",reloadIntervalMinutes:50,healthCheckIntervalSeconds:60,restartAfterHealthMisses:3,blockImagesAfterPlayback:true,blockFontsAfterPlayback:true,hideChatAfterPlayback:true,lowMemoryMode:true,memoryLimitMb:450},updates:{manifestUrl:location.origin+"/v1/update/manifest"}};
...
const merge=(base,extra)=>{const output=structuredClone(base);if(!extra||typeof extra!=="object"||Array.isArray(extra))return output;for(const [key,value] of Object.entries(extra)){if(value&&typeof value==="object"&&!Array.isArray(value)&&output[key]&&typeof output[key]==="object")output[key]=merge(output[key],value);else output[key]=value}output.cloudPollSeconds=60;output.telemetryMinutes=30;return output};
