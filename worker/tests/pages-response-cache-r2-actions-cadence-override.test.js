...
test('Actions cadence header is canonical', async () => {
  const now = Date.now();
  const response = await loadMaterializedR2Response({ get: async () => ({
    body: true, json: async () => ({ version: 1, updated_at: now, cadence_seconds: 60, headers: { 'x-materialized-cadence-seconds': '1' }, body: '{}' }),
