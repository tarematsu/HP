...
def resource_ids(account: str, queue_names: set[str]) -> tuple[set[str], set[str], set[str]]:
    ...
    missing_kv = set(KV_BINDINGS) - {
        str(row.get("name") or "") for row in bindings
        if str(row.get("type") or "") in {"kv_namespace", "kv_namespace_text"}
    }
    ...
    return queue_ids, namespace_ids, kv_ids
...
def self_test() -> int:
    ...
    assert "pipelinesOperatorAdaptiveGroups" not in document
    bindings = [{
    ...
    binding_names = ("RUNTIME_COORDINATOR",)
    assert durable_object_namespace_ids(bindings, namespaces, binding_names=binding_names) == {"own"}
    bindings_without_id = [{**bindings[0], "namespace_id": None}]
    assert durable_object_namespace_ids(
        bindings_without_id,
        namespaces,
...
def main() -> int:
    if "--self-test" in sys.argv:
        return self_test()
    if not TOKEN or not CONFIGS or not WORKER or not KV_BINDINGS or not DO_BINDINGS:
        raise RuntimeError("Cloudflare credentials, runtime Worker, config globs, and KV/DO bindings are required")
    account = account_id()
    queue_names, buckets = configured_resources()
    queue_ids, namespace_ids, kv_ids = resource_ids(account, queue_names)
    ...
    usage = aggregate(accounts[0], queue_ids, namespace_ids, buckets, kv_ids)
    ...
    report = {
        ...
        "policy": "80% of Cloudflare free/no-charge allowances for Queues, Durable Objects, R2, and KV",
    }
