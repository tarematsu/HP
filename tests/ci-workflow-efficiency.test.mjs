...
test('unified Cloudflare observability runs after either deploy and daily at 01:00 UTC', () => {
  ...
  assert.match(observability, /id: resolve-outcome/);
  assert.match(observability, /id: publish-status/);
  assert.match(observability, /id: publish-overall/);
  assert.match(observability, /continue-on-error: true/);
  assert.match(observability, /steps\.resolve-outcome\.outputs\.current_main_target == 'true'/);
  assert.match(observability, /steps\.resolve-outcome\.outputs\.overall == 'failure'/);
  assert.match(observability, /steps\.publish-status\.outcome == 'failure'/);
  assert.match(observability, /steps\.publish-overall\.outcome == 'failure'/);
