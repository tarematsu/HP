...
test('repairs the known 2024 range and leaves stored monthly summaries untouched', async () => {
  const f = fixture();
  ...
  assert.equal(f.db.prepare("SELECT stream_end FROM sh_monthly_summary WHERE period_key='2024-09'").get().stream_end, 200);
  assert.equal('monthly' in result, false);
});

test('known historical ranges overwrite daily closing values without updating monthly parents', async () => {
  const f = fixture();
  ...
  assert.equal(f.db.prepare("SELECT stream_end FROM sh_monthly_summary WHERE period_key='2025-08'").get().stream_end, 27850000);
  assert.equal(f.db.prepare("SELECT stream_end FROM sh_monthly_summary WHERE period_key='2025-09'").get().stream_end, 32050000);
