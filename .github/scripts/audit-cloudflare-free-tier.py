...
def self_test() -> int:
    ...
    try:
        ...
        assert paginated("account", "queues") == []
        assert calls == [
            f"{core.API}/accounts/account/queues?per_page=50&page=1"
        ]
    finally:
...
def main() -> int:
    ...
    queue_ids, namespace_ids, kv_ids = core.resource_ids(account, queue_names)
