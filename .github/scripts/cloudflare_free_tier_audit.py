...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"Could not load free-tier audit core from {CORE_PATH}")
...
def configured_resource_ids(queue_names: set[str]) -> tuple[set[str], set[str], set[str]]:
    """Use configured identities for report counts; GraphQL meters are account-wide."""
    return (
        set(queue_names),
        set(core.DO_BINDINGS),
        set(core.KV_BINDINGS),
    )
...
def aggregate(row: dict[str, Any]) -> dict[str, Any]:
    """Normalize account-wide groups into the validated resource aggregator."""
    normalized = dict(row)
    ...
    usage = _ORIGINAL_AGGREGATE(normalized, account, account, account, account)
...
def usage_basis(metric: str) -> str:
    ...
    if metric in {"r2ClassAOperations", "r2ClassBOperations"}:
        return "month-to-date"
    return "observed state"


def self_test() -> int:
    assert configured_resource_ids({"queue"}) == (
        {"queue"},
        set(core.DO_BINDINGS),
        set(core.KV_BINDINGS),
    )
    ...
    assert "pipelineId" not in document
...
def main() -> int:
    if (
        ...
        raise RuntimeError(
            "Cloudflare credentials, runtime Worker, config globs, and KV/DO bindings are required"
        )

    account = core.account_id()
    queue_names, buckets = core.configured_resources()
    queue_ids, namespace_ids, kv_ids = configured_resource_ids(queue_names)
