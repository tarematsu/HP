...
Create the GitHub Actions production secret `HOMEPANEL_RUNTIME_SECRETS_JSON` before cutover. Its value must be one JSON object containing the current runtime secret values from both legacy Workers, including `ADMIN_TOKEN` and the HomePanel device authentication secret(s).
...
7. Row counts, foreign keys, and SQLite integrity are verified.
8. The unified Worker is redeployed with video routes active.
9. Migration evidence is retained as a workflow artifact for 90 days.
