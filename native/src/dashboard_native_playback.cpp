...
std::wstring FetchPlaybackJson(const wchar_t* rawUrl, std::wstring* payload) {
  ...
  NativeHttpHandle session(WinHttpOpen(
      ...
      0));
  if (!session) return LastWinHttpError("WinHttpOpen");
  ...
  NativeHttpHandle connection(WinHttpConnect(
      ...
      parts.nPort,
      0));
  if (!connection) return LastWinHttpError("WinHttpConnect");
  ...
  if (!request) return LastWinHttpError("WinHttpOpenRequest");
  ...
  if (!WinHttpSendRequest(
    ...
    return LastWinHttpError("WinHttpReceiveResponse");
  }
