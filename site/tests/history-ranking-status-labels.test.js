...
test('leaderboard distinguishes the known collection gap from genuine out-of-rank weeks', () => {
...
test('leaderboard status labels target the visible rank column after fandom cleanup', () => {
  ...
  assert.match(statusSource, /cells\[rankIndex\]\.textContent = status/);
  assert.match(statusSource, /queueMicrotask\(\(\) => queueMicrotask\(applyStatusLabels\)\)/);
});

test('ranking status helper stays lazy and loads before the history data request', () => {
  assert.match(tabsSource, /history-ranking-table-status\.js\?v=20260923\.1/);
