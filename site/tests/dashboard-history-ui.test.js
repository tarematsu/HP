...
test('sparse daily summaries draw visible point markers in the dedicated chart', () => {
  assert.match(periodChart, /mode === 'daily' && lineCount === 0/);
  assert.match(periodChart, /context\.arc\(x, y, 3/);
  assert.doesNotMatch(historyFixes, /CanvasRenderingContext2D\.prototype|originalBeginPath|originalStroke/);
});
