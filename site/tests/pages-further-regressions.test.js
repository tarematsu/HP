...
test('track rows recover missing titles before rendering or session caching', () => {
  const rows = normalizeTrackRows([{ title: null, display_title: 'Recovered title' }]);
...
test('history patches cover session caches, full-row rankings and JST dates', () => {
  ...
  assert.match(fixes, /applyJstPreset/);
});

test('dashboard images retry transient failures', () => {
