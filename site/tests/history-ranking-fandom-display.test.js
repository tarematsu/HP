...
function rankingRows({ withFandom = true } = {}) {
  ...
  return [
    { ranking_date: '2026-09-07', ranking_type: '週間リーダーボード', rank: 10, host_name: 'sakuramankai', host_alias: 'sakuramankai', ...metadata },
    { ranking_date: '2026-09-21', ranking_type: '週間リーダーボード', rank: 8, host_name: 'sakuramankai', host_alias: 'sakuramankai', ...metadata },
  ];
}
...
function dbFor({ withFandom = true } = {}) {
  ...
  const gapMetadata = withFandom ? {
    artist_name: '櫻坂46', fandom_type: 'fandom', fandom_label: '櫻坂46(ファンダム)', stationhead_channel_name: 'Buddies',
  } : {
    artist_name: null, fandom_type: null, fandom_label: null, stationhead_channel_name: null,
  };
  const model = {
    version: 2,
    ...
    completed_rows: [
      rows[0],
      {
        ranking_date: '2026-09-14', observed_at: Date.parse('2026-09-14T00:00:00Z'), ranking_type: '週間リーダーボード',
        rank: null, host_name: 'sakuramankai', host_alias: 'sakuramankai', source_sheet: null, quality_score: null,
        quality_flags: 'not_listed', synthetic: true, is_out_of_rank: true, ...gapMetadata,
      },
      rows[1],
    ],
    weekly_metrics: [],
  ...
  return {
    prepare(sql) {
      ...
      return { async first() { return { payload_json: JSON.stringify(model), source_max_ranking_date: model.source_max_ranking_date, refreshed_at: model.refreshed_at }; } };
    },
  };
}
...
test('ranking API keeps materialized artist metadata and channel metadata on rows', async () => {
...
test('ranking table cleanup inserts channel, artist, and relation after host', () => {
...
test('D1 migration stores artist and relationship type and corrects sbuddies1819 idempotently', () => {
  const base = readFileSync(new URL('../../database/other-migrations/028_channel_fandom_metadata.sql', import.meta.url), 'utf8');
  const correction = readFileSync(new URL('../../database/other-migrations/039_correct_sbuddies1819_channel.sql', import.meta.url), 'utf8');
